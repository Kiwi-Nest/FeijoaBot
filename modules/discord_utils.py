...
async def ping_online_role(
    ...
    *,
    ...
    Args:
        role (discord.Role): The role to ping.
        user_db (UserDB): UserDB instance
        exclude_ids: Member IDs to exclude from the ping (e.g. users on bump cooldown).
    ...
    eligible = [m for m in role.members if m.id not in exclude_ids]

    # Prioritize online members who are also active.
    online_active_members = [
        member for member in eligible if member.status != discord.Status.offline and member.id in active_users_ids
    ]
    ...
    active_members = [member for member in eligible if member.id in active_users_ids]
